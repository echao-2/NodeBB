...
const socketHelpers = __importStar(require("../socket.io/helpers"));
const topics = __importStar(require("./index"));
const user = __importStar(require("../user"));
exports.Scheduled = {};
exports.Scheduled.startJobs = function () {
    winston.verbose('[scheduled topics] Starting jobs.');
    new cron_1.CronJob('*/1 * * * *', exports.Scheduled.handleExpired, null, true);
};
exports.Scheduled.handleExpired = function () {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield Promise.all([].concat(sendNotifications(uids, topicsData), updateUserLastposttimes(uids, topicsData), ...topicsData.map((topicData) => unpin(topicData.tid, topicData)), db.sortedSetsRemoveRangeByScore([`topics:scheduled`], '-inf', now)));
    });
};
// topics/tools.js#pin/unpin would block non-admins/mods, thus the local versions
exports.Scheduled.pin = function (tid, topicData) {
    return __awaiter(this, void 0, void 0, function* () {
        return Promise.all([
...
exports.Scheduled.reschedule = function ({ cid, tid, timestamp, uid }) {
    return __awaiter(this, void 0, void 0, function* () {
        yield Promise.all([
            db.sortedSetsAdd([
                'topics:scheduled',
...
function sendNotifications(uids, topicsData) {
    return __awaiter(this, void 0, void 0, function* () {
        const usernames = yield Promise.all(uids.map(uid => user.getUserField(uid, 'username')));
        ...
        return Promise.all(topicsData.map((t, idx) => user.notifications.sendTopicNotificationToFollowers(t.uid, t, postsData[idx])).concat(topicsData.map((t, idx) => socketHelpers.notifyNew(t.uid, 'newTopic', { posts: [postsData[idx]], topic: t }))));
    });
}
function updateUserLastposttimes(uids, topicsData) {
    return __awaiter(this, void 0, void 0, function* () {
        const lastposttimes = (yield user.getUsersFields(uids, ['lastposttime'])).map(u => u.lastposttime);
        let tstampByUid = {};
